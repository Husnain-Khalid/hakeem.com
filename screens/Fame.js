...
function Fame(props, navigation) {
  ...
  const fetchMedicine = async () => {
    await fetch(IP + 'hakeem/task')
      .then(response => response.json())
      .then(list => {
        console.log('listdadtdta', list);
                ...
                renderItem={({item, key}) => (
                  <View style={{flex: 1}}>
                    <TouchableOpacity
                      ...
                      <Text
                        style={{fontSize: 24, fontFamily: 'bold', height: 60}}>
                        {'Medicine Name: '}
                        {item.title}
                      </Text>
...
export default withInAppNotification(Fame);
